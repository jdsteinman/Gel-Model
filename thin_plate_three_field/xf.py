...
if rank == 0:
    ...
    if not os.path.exists(res_dir):
        os.makedirs(res_dir)
        shutil.copyfile('xf.py',os.path.join(res_dir,'xf.py'))

mesh = df.Mesh("./meshes/plate_with_hole.xml")
for n in range(0):
    mesh = df.refine(mesh)
...
u_inner = df.Expression(("0.2*t*x[0]","-0.5*t*x[1]"),t=0,degree=2)
...
'''
bc1 = df.DirichletBC(W.sub(0), df.Constant((0.0, 0.0)), boundaries, 1)
bc2 = df.DirichletBC(W.sub(0), df.Constant((0.0, 0.0)), boundaries, 2)
bc3 = df.DirichletBC(W.sub(0), u_inner, boundaries, 3)
bc4 = df.DirichletBC(W.sub(0), df.Constant((0.0, 0.0)), boundaries, 4)
bcs = [bc3]
